...
from ui.setTunnelUI import Ui_MainWin
...
class setTunnel(QtWidgets.QMainWindow, Ui_MainWin):
    ...
    def json_is_null(self):
        ...
        if len(target_info) == 0:
            reply = QtWidgets.QMessageBox.question(self, '警告', "Target Server 一个都没有，你只能留在这", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
        ...
        if len(proxy_info) == 0:
            reply = QtWidgets.QMessageBox.question(self, '警告', "Proxy Server 一个都没有，你只能留在这", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                                                       QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.Yes:
                # self.stackedWidget.setCurrentIndex(1)
                self.fun_page_add()
    ...
    def update_available_server(self):
        ...
        self.tblWdgAvailableTargetServer.setHorizontalHeaderLabels(['Name', 'Target Host:Port', 'Local Port'])
        server_data, target_info, proxy_info = self.load_server_json()
        try:
            for idx, each_target in enumerate(target_info.items()):
                ...
                target_server_item = QTableWidgetItem("{}:{}".format(str(cryptocode.decrypt(each_target[1]["Host"], self.other_key)), str(each_target[1]["Port"])))
                ...
                self.tblWdgAvailableTargetServer.setItem(idx, 1, target_server_item)
                self.tblWdgAvailableTargetServer.setItem(idx, 2, local_port_item)
        ...
        # self.tblWdgStatus.setHorizontalHeaderLabels(['Local Port', 'SSH Method', 'Target Host', 'Target Port', 'Proxy Host', 'Proxy Port', 'Proxy User'])
        self.tblWdgAvailableProxyServer.setHorizontalHeaderLabels(['Name', 'UserName', 'Proxy Host:Port'])
        server_data, target_info, proxy_info = self.load_server_json()
        try:
            for idx, each_proxy in enumerate(proxy_info.items()):
                ...
                proxy_server_item = QTableWidgetItem("{}:{}".format(str(cryptocode.decrypt(each_proxy[1]["Host"], self.other_key)), str(each_proxy[1]["Port"])))
    ...
    def update_connect_status(self):
        ...
        # self.tblWdgStatus.setHorizontalHeaderLabels(['Local Port', 'SSH Method', 'Target Host', 'Target Port', 'Proxy Host', 'Proxy Port', 'Proxy User'])
        self.tblWdgStatus.setHorizontalHeaderLabels(['Local Port', 'SSH Method', 'Target Server', 'Proxy Server'])
        for idx, each_tunnel in enumerate(self.tunnel_start_status):
            ...
            target_server_item = QTableWidgetItem('{}:{}'.format(str(each_tunnel["Target_Host"]), str(each_tunnel["Target_Port"])))
            target_server_item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            proxy_server_item = QTableWidgetItem('{}@{}:{}'.format(str(each_tunnel["Proxy_User"]), str(each_tunnel["Proxy_Host"]), str(each_tunnel["Proxy_Port"])))
            proxy_server_item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
    ...
    def fun_add_target(self):
        ...
        if len(name) == 0 or len(host) == 0 or len(port) == 0 or len(localport) == 0:
            ...
            for each_target in target_info.items():
                ...
                #     if int(port) == int(each_target[1]["Port"]):
                #         不允许同一个Host:Port
                #         al_count += 1
    ...
    def fun_revise_target(self):
        ...
        if len(name) == 0 or len(host) == 0 or len(port) == 0 or len(localport) == 0:
            ...
            if al_count > 0:
                self.txtBrowerReviseTargetStatus.setText(
                    "<font color='red'> 存在相同的'LocalPort'或和其他'Name'冲突, 修改失败 <font>")
            else:
